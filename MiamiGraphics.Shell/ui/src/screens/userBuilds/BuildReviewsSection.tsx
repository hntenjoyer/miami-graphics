...
import type { UserBuildReview } from '@/bridge/types';
...
export function BuildReviewsSection({ buildId }: Props) {
  ...
  const [reviews, setReviews]       = useState<UserBuildReview[]>([]);
  const [loading, setLoading]       = useState(true);
  const [error, setError]           = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [editing, setEditing]       = useState(false);
  const [pendingDelete, setPendingDelete] = useState<UserBuildReview | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await bridge.userBuildReviewsList(buildId);
        if (!cancelled) setReviews(list);
      } catch (e) {
        if (!cancelled) { console.warn('[buildReviews] list failed:', e); setReviews([]); }
      } finally {
  ...
  const canDelete = (review: UserBuildReview): boolean =>
  ...
  const deleteReview = async (reviewId: string) => {
    ...
    setError(null);
    try {
      const ok = await bridge.userBuildReviewDelete(reviewId, userId, role);
      if (!ok) { setError(t('reviews.deleteFailed', 'Не удалось удалить отзыв')); return; }
      setReviews(prev => prev.filter(r => r.id !== reviewId));
    } catch (e) {
      setError(e instanceof Error ? e.message : t('reviews.deleteFailed', 'Не удалось удалить отзыв'));
    }
  };
  ...
  const submit = async (rating: number, body: string) => {
    ...
    setError(null);
    try {
      const fresh = await bridge.userBuildReviewSubmit(buildId, userId, username, role, myAvatar, rating, body);
      setReviews(prev => [fresh, ...prev.filter(r => r.userId !== fresh.userId)]);
      setEditing(false);
    } catch (e) {
      setError(e instanceof Error ? e.message : t('reviews.submitFailed', 'Не удалось отправить отзыв'));
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="flex flex-col gap-5">
      <header className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-[10px] uppercase tracking-[0.28em] text-text-muted mt-1.5">
            {t('reviews.subtitle', 'Мнения сообщества')}
          </p>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-8 items-center">
          <div className="text-center md:text-left md:pr-8 md:border-r md:border-glass-border">
            ...
            <div className="flex items-center justify-center md:justify-start gap-1 mt-3">
              {[1, 2, 3, 4, 5].map(n => (
                <Star key={n} size={20}
                  className={n <= Math.round(stats.avg) ? 'text-yellow-400 fill-current' : 'text-text-muted/30'} />
              ))}
            </div>
            <div className="text-[10px] uppercase tracking-[0.2em] text-text-muted mt-3">
              {t('reviews.totalCount', '{{count}} отзывов', { count: stats.total })}
            </div>
          </div>
              ...
              return (
                <div key={rating} className="flex items-center gap-3 text-sm">
                  ...
                  <div className="flex-1 h-2 rounded-full bg-glass-strong border border-glass-border overflow-hidden">
                    <motion.div initial={false} animate={{ width: `${pct}%` }}
      ...
      {loading ? (
        <GlassPanel depth="z1" tint="ultra" highlight rounded="3xl" className="p-7">
          <p className="text-sm text-text-muted text-center">{t('reviews.loading', 'Загрузка...')}</p>
        </GlassPanel>
      ) : reviews.length === 0 ? (
        <GlassPanel depth="z1" tint="ultra" highlight rounded="3xl" className="p-7">
          <p className="text-sm text-text-muted text-center">{t('reviews.empty', 'Пока нет отзывов. Будь первым!')}</p>
        </GlassPanel>
      ) : (
        <div className="flex flex-col gap-3">
          <AnimatePresence initial={false}>
            {reviews.map(r => (
              ...
              >
                <ReviewItem review={r} canDelete={canDelete(r)} onDelete={() => setPendingDelete(r)} />
              </motion.div>
            ))}
          </AnimatePresence>
        </div>
      ...
      <ConfirmModal
        ...
        title={t('reviews.deleteConfirmTitle', 'Удалить отзыв?')}
        message={pendingDelete ? `${pendingDelete.username}\n\n${t('reviews.deleteConfirmBody', 'Это действие нельзя отменить.')}` : ''}
        confirmLabel={t('reviews.delete', 'Удалить')}
        cancelLabel={t('reviews.cancel', 'Отмена')}
        destructive
        onConfirm={() => { if (pendingDelete) void deleteReview(pendingDelete.id); setPendingDelete(null); }}
...
function OwnReviewSuccess({ onEdit }: { onEdit: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
      ...
      <h3 className="font-bold text-text-primary text-lg mb-1">{t('reviews.posted', 'Отзыв опубликован')}</h3>
      <p className="text-sm text-text-muted mb-5">{t('reviews.thanks', 'Спасибо за обратную связь!')}</p>
      <button type="button" onClick={onEdit}
        className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl
                   bg-glass-strong border border-glass-border text-sm text-text-primary
                   hover:bg-glass transition-colors duration-200">
        <Pencil size={13} />
        {t('reviews.edit', 'Изменить')}
      </button>
    </motion.div>
  );
}
...
function ReviewForm({ initial, isEdit, busy, onSubmit, onCancel }: FormProps) {
  ...
  const handle = (e: React.FormEvent) => { e.preventDefault(); if (canSubmit) onSubmit(rating, body); };

  return (
    <form onSubmit={handle} className="flex flex-col gap-3">
      <div className="flex items-center gap-2">
        <span className="text-sm text-text-secondary mr-1">
          {isEdit ? t('reviews.yourRatingEdit', 'Твоя оценка') : t('reviews.yourRating', 'Оценка')}:
        </span>
        {[1, 2, 3, 4, 5].map(n => {
          ...
          return (
            <button key={n} type="button"
              ...
              onClick={() => setRating(n)} className="p-1 rounded-md transition-colors"
              aria-label={`${n}`}>
              <Star size={22} className={filled ? 'text-yellow-400 fill-current' : 'text-text-muted/40'} />
            </button>
          );
        })}
      </div>
      ...
      <textarea
        ...
        placeholder={t('reviews.placeholder', 'Расскажи, как тебе сборка...')}
        rows={3}
      ...
      <div className="flex justify-end gap-2">
        {onCancel && (
          <button type="button" onClick={onCancel}
            ...
            <X size={14} />
            {t('reviews.cancel', 'Отмена')}
          </button>
        )}
        <button type="submit" disabled={!canSubmit || busy} style={{ outline: 'none' }}
          ...
          <span>{isEdit ? t('reviews.update', 'Обновить') : t('reviews.submit', 'Отправить')}</span>
        </button>
      </div>
    </form>
  );
...
function SignInPrompt() {
  const { t } = useTranslation();
  return (
    <div className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-glass-strong border border-glass-border">
      <LogIn size={16} className="text-text-muted shrink-0" />
      <span className="text-sm text-text-secondary">
        {t('reviews.guestPrompt', 'Войди в аккаунт, чтобы оставить отзыв')}
      </span>
    </div>
  );
}
      ...
      <div className="flex gap-4">
        {review.avatarUrl ? (
          <img src={review.avatarUrl} alt={review.username}
            className="shrink-0 w-11 h-11 rounded-full object-cover border border-glass-border shadow-z1 bg-glass-strong"
            onError={(e) => {
              const el = e.currentTarget;
              const fb = document.createElement('div');
              fb.className = 'shrink-0 w-11 h-11 rounded-full bg-glass-strong border border-glass-border ' +
                'flex items-center justify-center font-bold text-sm text-text-primary shadow-z1';
              fb.textContent = initial;
              el.replaceWith(fb);
            }} />
        ) : (
          <div className="shrink-0 w-11 h-11 rounded-full bg-glass-strong border border-glass-border
                          flex items-center justify-center font-bold text-sm text-text-primary shadow-z1">
            {initial}
          </div>
        )}
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-3 flex-wrap">
            <span className="text-sm font-bold text-text-primary truncate tracking-tight">{review.username}</span>
            <span className="text-[11px] text-text-muted tabular-nums ml-auto">{dateLabel}</span>
            {canDelete && (
              <button type="button" onClick={() => onDelete?.()}
                title={t('reviews.delete', 'Удалить')} aria-label={t('reviews.delete', 'Удалить')}
                style={{ outline: 'none' }}
