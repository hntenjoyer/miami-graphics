...
import type { BigMapReview } from '@/bridge/types';
...
export function BigMapReviewsSection({ mapId }: Props) {
  ...
  const [reviews, setReviews]       = useState<BigMapReview[]>([]);
  const [loading, setLoading]       = useState(true);
  const [error, setError]           = useState<string | null>(null);
  const [submitting, setSubmitting] = useState(false);
  const [editing, setEditing]       = useState(false);

  const [pendingDelete, setPendingDelete] = useState<BigMapReview | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const list = await bridge.bigMapReviewsList(mapId);
  ...
  const canDelete = (review: BigMapReview): boolean => {
    if (isAdmin) return true;
    return !!userId && review.userId === userId;
  };

  const refreshRatings = useBigMapStore(s => s.loadRatings);

  const deleteReview = async (reviewId: string) => {
    if (!userId) return;
    setError(null);
    try {
      const ok = await bridge.bigMapReviewDelete(reviewId, userId, role);
      if (!ok) {
  ...
  const submit = async (rating: number, body: string) => {
    ...
    setError(null);
    try {
      const fresh = await bridge.bigMapReviewSubmit(mapId, userId, username, role, myAvatar, rating, body);
