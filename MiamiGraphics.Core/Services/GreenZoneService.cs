...
{
    public static class GreenZoneService
    {
        public const string FileName        = "gz.rpf";
        public const string FolderName       = "green_zone";
        public const string EnableItem       = "update:/green_zone/gz.rpf";
        ...
        private const string PathSuffix      = "/green_zone/gz.rpf";
        private const string LegacyFileName   = "zz_rpf.rpf";
        private const string LegacyPathSuffix = "/green_zone/zz_rpf.rpf";

        public static bool Apply(string updateRpfPath, byte[] gzRpfBytes)
        {
            if (gzRpfBytes is null || gzRpfBytes.Length == 0)
                throw new ArgumentException("gz.rpf bytes empty", nameof(gzRpfBytes));
            return Mutate(updateRpfPath, enable: true, gzRpfBytes);
        }

        public static bool Remove(string updateRpfPath)
            => Mutate(updateRpfPath, enable: false, gzRpfBytes: null);
        ...
        public static bool IsApplied(string updateRpfPath)
        {
            try
            {
                ...
                bool fileThere = FindGreenZoneDir(arc.Root, createMissing: false)?
        ...
        private static bool Mutate(string updateRpfPath, bool enable, byte[]? gzRpfBytes)
        {
            ...
            var doc = XDocument.Parse(DecodeText(ReadEntryDecoded(contentEntry)));
            if (enable) AddEntries(doc); else RemoveEntries(doc);
            RemoveEntriesBySuffix(doc, LegacyPathSuffix);
            ...
            var gzDir = FindGreenZoneDir(arc.Root, createMissing: enable);
            if (gzDir is null)
            {
                if (enable) throw new InvalidOperationException("green_zone directory could not be created in update.rpf.");
            }
            else
            {
                var existing = gzDir.GetFiles()
                    .FirstOrDefault(f => f.Name.Equals(FileName, StringComparison.OrdinalIgnoreCase));
                if (enable)
                {
                    if (existing is IArchiveBinaryFile eb)
                    {
                        eb.Import(new MemoryStream(gzRpfBytes!));
                        eb.IsCompressed = false;
                        eb.IsEncrypted = false;
                        eb.UncompressedSize = (uint)gzRpfBytes!.Length;
                    }
                    else
                    {
                        var nf = gzDir.CreateBinaryFile();
                        nf.Name = FileName;
                        nf.Import(new MemoryStream(gzRpfBytes!));
                        nf.IsCompressed = false;
                        nf.IsEncrypted = false;
                        nf.UncompressedSize = (uint)gzRpfBytes!.Length;
                    }
                }
                else if (existing is not null)
                {
                    try { gzDir.DeleteFile(existing); }
                    catch {}
                }

                var legacy = gzDir.GetFiles()
                    .FirstOrDefault(f => f.Name.Equals(LegacyFileName, StringComparison.OrdinalIgnoreCase));
                if (legacy is not null)
                { try { gzDir.DeleteFile(legacy); } catch {} }
            }
        ...
        private static bool RemoveEntries(XDocument doc) => RemoveEntriesBySuffix(doc, PathSuffix);
        ...
        private static IArchiveDirectory? FindGreenZoneDir(IArchiveDirectory root, bool createMissing)
        {
