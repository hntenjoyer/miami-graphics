...
public static class AccessoryService
{
        ...
        float size, float depthFrac, string texName, bool glbSpace = true)
    {
        ...
        if (mesh == null) throw new InvalidOperationException(Loc.T("error.outlineFromPngFailedHint"));
    ...
    {
        int vc = pos.Length / 3;
        if (vc == 0) throw new InvalidOperationException(Loc.T("error.emptyGeometry"));
        if (vc > 65000) throw new InvalidOperationException(Loc.T("error.modelTooDetailedLimit", ("verts", vc)));
        if (idx.Length % 3 != 0) throw new InvalidOperationException(Loc.T("error.indicesNotMultipleOf3"));
        ...
        List<MeshVert> verts, List<ushort> indices)
    {
        ...
        foreach (var file in Directory.GetFiles(gunDir, "*.ydr"))
        {
            ...
            ydr.Load(File.ReadAllBytes(file));
            var d = ydr.Drawable;
            if (d?.ShaderGroup == null || d.DrawableModels?.High == null || d.DrawableModels.High.Length == 0)
                continue;
        ...
        string texName, List<MeshVert> verts, List<ushort> indices)
    {
        ...
        if (td == null)
        {
            td = new TextureDictionary();
            d.ShaderGroup.TextureDictionary = td;
        }
        var texList = (td.Textures?.data_items ?? Array.Empty<Texture>()).ToList();
        var donorTex = texList.FirstOrDefault();

        var enc = MiamiGraphics.Core.Services.GameTextureWriter.Encode(bgra, tw, th);
        var tex = new Texture
        {
            Name = texName,
            NameHash = JenkHash.GenHash(texName.ToLowerInvariant()),
            Width = (ushort)enc.Width,
            Height = (ushort)enc.Height,
            Depth = 1,
            Levels = enc.Levels,
            Format = enc.Format,
            Stride = enc.Stride,
