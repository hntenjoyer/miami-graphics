...
public static class AnimatedDetailService
{
    ...
    private const uint ArchetypeFlags = 525824;
    ...
    public sealed class GenRequest
    {
        ...
        public string AttachBone = "gun_root";
    ...
    public sealed class GameLooseEntry
    {
    ...
    public sealed class UpdateRpfPatchFile
    {
    ...
    public sealed class GameInstallSet
    {
        public Dictionary<string, byte[]> StreamedFiles { get; } = new(StringComparer.OrdinalIgnoreCase);
        public List<GameLooseEntry> LooseFiles { get; } = new();
        public UpdateRpfPatchFile UpdateRpfMeta { get; set; }
        public string ComponentName { get; set; }
        public string ModelName { get; set; }
        public string WeaponKey { get; set; }
        public string PreviewGlb { get; set; }
        public AnimMeta Anim { get; set; }
    }

    private sealed record WeaponRecipe(string Key, string ComponentName, string ModelName,
        string MetaResource, string MetaFileName, string HomeDlc);

    private static readonly WeaponRecipe[] Recipes =
    {
        new("specialcarbinemk2", "SPECIALMK2",     "speshik",        "weapons_specialcarbine_mk2.meta", "weapons_specialcarbine_mk2.meta", "mpchristmas2017"),
        new("carbineriflemk2",   "CARBMK2ANIM",    "mgd_carbmk2",    "weapons_carbinerifle_mk2.meta",   "weapons_carbinerifle_mk2.meta",   "mpgunrunning"),
        new("marksmanriflemk2",  "MARKSMK2ANIM",   "mgd_marksmk2",   "weapons_marksmanrifle_mk2.meta",  "weapons_marksmanrifle_mk2.meta",  "mpchristmas2017"),
        new("heavysnipermk2",    "HSNIPMK2ANIM",   "mgd_hsnipmk2",   "weapons_heavysniper_mk2.meta",    "weapons_heavysniper_mk2.meta",    "mpgunrunning"),
        new("combatmgmk2",       "CMGMK2ANIM",     "mgd_cmgmk2",     "weapons_combatmg_mk2.meta",       "weapons_combatmg_mk2.meta",       "mpgunrunning"),
        new("smgmk2",            "SMGMK2ANIM",     "mgd_smgmk2",     "weapons_smg_mk2.meta",            "weapons_smg_mk2.meta",            "mpgunrunning"),
        new("heavyrifle",        "HEAVYRIFLEANIM", "mgd_heavyrifle", "weapons_heavyrifle.meta",         "weapons_heavyrifle.meta",         "mpsecurity"),
        new("precisionrifle",    "PRECISIONANIM",  "mgd_precision",  "weapon_precisionrifle.meta",      "weapon_precisionrifle.meta",      "mpsum2"),
        new("revolver",          "REVOLVERANIM",   "mgd_revolver",   "weaponrevolver.meta",             "weaponrevolver.meta",             "mpapartment"),
        new("minismg",           "MINISMGANIM",    "mgd_minismg",    "weaponminismg.meta",              "weaponminismg.meta",              "mpbiker"),
    };

    private static WeaponRecipe FindRecipe(string gunKey)
    {
        string k = (gunKey ?? "").ToLowerInvariant().Replace("_", "").Replace(" ", "");
        return Recipes.FirstOrDefault(r => k.Contains(r.Key));
    }

    public static bool IsInstallSupported(string gunKey) => FindRecipe(gunKey) != null;

    public static string SupportedListText => Loc.T("misc.animDetailSupportedGuns");

    private static byte[] ReadEmbeddedMeta(string fileName)
    {
        var asm = typeof(AnimatedDetailService).Assembly;
        var res = asm.GetManifestResourceNames()
            .FirstOrDefault(n => n.EndsWith("." + fileName, StringComparison.OrdinalIgnoreCase)
                              || n.EndsWith(fileName, StringComparison.OrdinalIgnoreCase));
        if (res == null) throw new InvalidOperationException(Loc.T("error.embeddedMetaMissing", ("file", fileName)));
    ...
    public static GameInstallSet BuildGameInstall(GenRequest req, string gunKey, string previewGlbDir = null)
    {
        var recipe = FindRecipe(gunKey)
            ?? throw new ArgumentException(Loc.T("error.gunNotSupportedForInstall",
                ("gun", gunKey), ("supported", SupportedListText)));

        string modelName = recipe.ModelName;
        ...
        if (verts.Count == 0) throw new InvalidOperationException(Loc.T("error.emptyDetailGeometry"));
        ...
        string ycdXml = req.AnimMode switch
        {
            "swing" => BuildBoneRotYcdXml(modelName, MovableBoneTag, req.PeriodSec, frames,
                            new Vector3(req.AxisX, req.AxisY, req.AxisZ), req.AmplitudeDeg, spin: false, 1f),
            "spin" => BuildBoneRotYcdXml(modelName, MovableBoneTag, req.PeriodSec, frames,
                            new Vector3(req.AxisX, req.AxisY, req.AxisZ), 0f, spin: true, 1f),
            _ => BuildUvScrollYcdXml(modelName, req.PeriodSec, frames, req.ScrollU, req.ScrollV, 1f),
        };
        byte[] ycdBytes = XmlYcd.GetYcd(ycdXml).Save();
        byte[] ytypBytes = BuildYtyp(modelName, dictName, bbMin, bbMax, bsCentre, bsRadius);
        byte[] setcompsBytes = new UTF8Encoding(false).GetBytes(
            BuildComponentMeta(recipe.ComponentName, modelName, "AAPCover"));
        byte[] weaponMetaBytes = ReadEmbeddedMeta(recipe.MetaResource);
        {
            string bone = string.IsNullOrWhiteSpace(req.AttachBone) ? "gun_root" : req.AttachBone;
            string txt = new UTF8Encoding(false).GetString(weaponMetaBytes);
            if (txt.Contains("MG_ANIM_BONE"))
                txt = txt.Replace("<AttachBone>MG_ANIM_BONE</AttachBone>", $"<AttachBone>{bone}</AttachBone>");
            else if (!bone.Equals("gun_root", StringComparison.OrdinalIgnoreCase))
                txt = txt.Replace("<AttachBone>gun_root</AttachBone>", $"<AttachBone>{bone}</AttachBone>");
            weaponMetaBytes = new UTF8Encoding(false).GetBytes(txt);
        }

        var set = new GameInstallSet
        {
            ComponentName = recipe.ComponentName,
            ModelName = modelName,
            WeaponKey = recipe.Key,
            Anim = new AnimMeta
            {
                Mode = req.AnimMode, PeriodSec = req.PeriodSec, ScrollU = req.ScrollU, ScrollV = req.ScrollV,
                Axis = new[] { req.AxisX, req.AxisY, req.AxisZ }, AmplitudeDeg = req.AmplitudeDeg, AttachBone = "gun_root",
            },
        };
        set.StreamedFiles[modelName + ".ydr"] = ydrBytes;
        set.StreamedFiles[dictName + ".ycd"] = ycdBytes;
        set.StreamedFiles[dictName + ".ytyp"] = ytypBytes;
        set.LooseFiles.Add(new GameLooseEntry
        {
            RelPath = "%PLATFORM%/levels/gta5/" + dictName + ".ityp",
            Bytes = null,
            FileType = "DLC_ITYP_REQUEST",
            Contents = "CONTENTS_PROPS",
        });
        set.LooseFiles.Add(new GameLooseEntry { RelPath = "setcomps_" + modelName + ".meta", Bytes = setcompsBytes, FileType = "WEAPONCOMPONENTSINFO_FILE" });
        set.UpdateRpfMeta = new UpdateRpfPatchFile
        {
            DlcName = recipe.HomeDlc,
            RelPath = "common/data/ai/" + recipe.MetaFileName,
            Bytes = weaponMetaBytes,
        };

        if (!string.IsNullOrEmpty(previewGlbDir))
        {
            try
            {
                Directory.CreateDirectory(previewGlbDir);
                string ydrTmp = Path.Combine(previewGlbDir, modelName + ".ydr");
                File.WriteAllBytes(ydrTmp, ydrBytes);
                string glb = Path.Combine(previewGlbDir, modelName + ".glb");
                if (MiamiGraphics.Core.Services.YdrToGltfConverter.ConvertAsync(ydrTmp, glb, new List<string>()).GetAwaiter().GetResult())
                    set.PreviewGlb = modelName + ".glb";
            }
            catch { }
        }
        return set;
        ...
        List<AccessoryService.MeshVert> verts, List<ushort> idx)
    {
            ...
            req.Pos != null && req.Pos.Length >= 9)
        {
            ...
            if (vc > 65000) throw new InvalidOperationException(Loc.T("error.modelTooDetailed", ("verts", vc)));
            for (int i = 0; i < vc; i++)
            {
            ...
            ApplyPlacement(req, verts);
            if (req.Mirror) AppendMirror(verts, idx);
            return;
        }

        var mesh = ExtrudeService.Build(bgra, tw, th, req.Size,
            req.Size * Math.Clamp(req.DepthFrac, 0.03f, 0.6f));
        if (mesh == null) throw new InvalidOperationException(Loc.T("error.outlineFromPngFailed"));
    ...
    private static void AppendMirror(List<AccessoryService.MeshVert> verts, List<ushort> idx)
    {
        int baseV = verts.Count, baseI = idx.Count;
        if (baseV == 0 || baseV * 2 > 65000) return;
        for (int i = 0; i < baseV; i++)
        {
            var v = verts[i];
            var p = new Vector3(-v.P.X, v.P.Y, v.P.Z);
            var n = new Vector3(-v.N.X, v.N.Y, v.N.Z);
            verts.Add(new AccessoryService.MeshVert(p, n, v.U, v.V));
        }
        for (int i = 0; i + 2 < baseI; i += 3)
        {
            ushort a = idx[i], b = idx[i + 1], c = idx[i + 2];
            idx.Add((ushort)(baseV + a));
            idx.Add((ushort)(baseV + c));
            idx.Add((ushort)(baseV + b));
        }
    }

    private static void ApplyPlacement(GenRequest req, List<AccessoryService.MeshVert> verts)
    {
        bool rot = req.RotX != 0f || req.RotY != 0f || req.RotZ != 0f;
        bool off = req.OffX != 0f || req.OffY != 0f || req.OffZ != 0f;
        if (!rot && !off) return;
        const float D2R = (float)(Math.PI / 180.0);
        var m = Matrix.RotationYawPitchRoll(req.RotY * D2R, req.RotX * D2R, req.RotZ * D2R);
        var offset = new Vector3(req.OffX, req.OffY, req.OffZ);
        for (int i = 0; i < verts.Count; i++)
        {
            var v = verts[i];
            var p = Vector3.TransformCoordinate(v.P, m) + offset;
            var n = Vector3.TransformNormal(v.N, m);
            if (n.LengthSquared() > 1e-8f) n.Normalize();
            verts[i] = new AccessoryService.MeshVert(p, n, v.U, v.V);
        }
    }
    ...
    private static ShaderFX BuildEmissiveAnimShader(Texture tex)
    {
        ...
        var names = new[]
        {
            ...
            ShaderParamNames.wetnessMultiplier,
    ...
    private static Texture BuildTexture(string name, byte[] bgra, int w, int h)
    {
        var enc = MiamiGraphics.Core.Services.GameTextureWriter.Encode(bgra, w, h);
        return BuildTexture(name, enc);
    }

    private static Texture BuildTexture(string name,
        MiamiGraphics.Core.Services.GameTextureWriter.Encoded enc) => new()
    {
        Name = name,
        NameHash = JenkHash.GenHash(name.ToLowerInvariant()),
        Width = (ushort)enc.Width, Height = (ushort)enc.Height, Depth = 1, Levels = enc.Levels,
        Format = enc.Format,
        Stride = enc.Stride,
        Data = new TextureData { FullData = enc.Data },
