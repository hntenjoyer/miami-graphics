...
namespace MiamiGraphics.Core.System
{
    public static class FeatureLog
    {
        ...
        public static string LogPath => Path.Combine(
            ...
            "MiamiGraphics", "logs", "features.log");

        public static string PrevLogPath => Path.Combine(
            Path.GetDirectoryName(LogPath)!, "features.prev.log");

        private static readonly Mutex Gate = CreateGate();

        private static Mutex CreateGate()
        {
            try { return new Mutex(false, @"Local\MiamiGraphicsFeatureLog"); }
        ...
        private static void RotateIfNeeded()
        {
            try
            {
                ...
                try { File.Delete(PrevLogPath); } catch { }
