    ...
    public class XmlPso
    {
        ...
        private static void TraverseMap(XmlNode node, PsoBuilder pb, PsoStructureEntryInfo entry, PsoStructureInfo infos, byte[] data, PsoArrayResults arrayResults)
        {
            ...
            foreach (XmlNode cnode in node.ChildNodes)
            {
                ...
                var tattr = cnode.Attributes["type"].Value;
                ...
                TraverseStringRaw(kattr, pb, kEntry, nodeBytes);

                if (xName != (MetaName)MetaTypeName.ARRAYINFO)
                {
            ...
            arrayResults.Structures[entry.DataOffset + 8] = pb.AddItemArrayPtr(xName, nodesData.ToArray());
        }
                ...
                case PsoDataType.Enum:
                    {
                        ...
                        if (arrEntry.ReferenceKey != 0)
                        {
                            ...
                            {
                                var enumname = (MetaName)MetaTypes.SwapBytes(hashes[i]);
                ...
                case PsoDataType.Array:
                    {
                        ...
                        if (rk0 > 0)
                        {
                            ...
                            arrEntry.DataOffset = entry.DataOffset;
                            foreach (XmlNode cnode in node.ChildNodes)
                            {
                                if (cnode.NodeType != XmlNodeType.Element) continue;
