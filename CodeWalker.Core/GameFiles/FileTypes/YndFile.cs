...
namespace CodeWalker.GameFiles
{
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YndFile : GameFile, PackedFile, BasePathData
    {
        ...
        public byte[] Save()
        {
            ...
            byte[] data = ResourceBuilder.Build(NodeDictionary, 1);
            ...
            if ((Links != null) && (Nodes != null))
            {
                foreach (var node in Nodes)
                {
                    ...
                    foreach (var link in node.Links)
                    {
                        ...
                        float inner = link.LaneOffset * lanewidth;
        ...
        private void UpdateJunctionTriangleVertices(YndNode[] selectedNodes)
        {
            ...
            foreach (var node in selectedNodes)
            {
                ...
                for (int y = 1; y < d.CountY; y++)
                {
                    var row0 = d.Rows[y - 1];
                    var row1 = d.Rows[y];
                    float offy = y * 2.0f;

                    for (int x = 1; x < d.CountX; x++)
                    {
