...
namespace CodeWalker.GameFiles
{
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvFile : GameFile, PackedFile, BasePathData
    {
        ...
        public byte[] Save()
        {
            ...
            byte[] data = ResourceBuilder.Build(Nav, 2);
        ...
        private void BuildStructs()
        {
            ...
            if (Polys != null)
            {
                for (int i = 0; i < Polys.Count; i++)
                {
                    ...
                    poly._RawData.PortalLinkID = (uint)portallinks.Count;
            ...
            for (int i = 0; i < Nav.Polys.ListParts.Count; i++)
            {
        ...
        private void BuildSectorTree(NavMeshSector node, int depth, ref uint pointindex, bool[] pointflags)
        {
            ...
            else
            {
                ...
                node.SubTree1.SetAABBs(new Vector3(cen.X, cen.Y, cen.Z), new Vector3(max.X, max.Y, max.Z));
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvPoly : IMetaXmlItem
    {
        ...
        public Color4 GetColour()
        {
            ...
            if ((u0 & 1) > 0) colour.Red += 0.01f;
            if ((u0 & 2) > 0) colour.Red += 0.01f;
            if ((u0 & 4) > 0) colour.Green += 0.25f;
            if ((u0 & 8) > 0) colour.Green += 0.02f;
            if ((u0 & 64) > 0) colour.Red += 0.25f;
            if ((u0 & 128) > 0) colour.Blue += 0.25f;

            var u2 = _RawData.PolyFlags1;
            if ((u2 & 64) > 0) colour.Blue += 0.1f;
            if ((u2 & 512) > 0) colour.Green += 0.1f;
            if ((u2 & 1024) > 0) colour.Blue += 0.03f;
            if ((u2 & 4096) > 0) colour.Green += 0.75f;
            if ((u2 & 8192) > 0) colour.Blue += 0.75f;
            if ((u2 & 16384) > 0) colour.Red += 0.2f;
            if ((u2 & 32768) > 0) colour.Blue += 0.2f;
            if ((u2 & 65536) > 0) colour.Green = 0.2f;

            var u5 = _RawData.PolyFlags2;
        ...
        public void ReadXml(XmlNode node)
        {
            ...
            Indices = new ushort[Vertices?.Length ?? 0];
