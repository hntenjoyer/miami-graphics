...
namespace CodeWalker.Core.GameFiles.FileTypes.Builders
{
    public class YnvBuilder
    {
        ...
        private List<YnvPoly> SplitPolys(List<YnvPoly> polys, bool xaxis)
        {
            ...
            foreach (var polysplit in polysplits.Values)
            {
                ...
                var se1 = edges[split1beg];
                var se2 = edges[split2beg];
                var sp1 = TryGetSplit(polysplits, se1.Poly1);
                ...
                for (int i = 0; i < split1beg; i++) edges1.Add(edges[i]);
                edges1.Add(edge1a);
                edges1.Add(splita);
                edges1.Add(edge2a);
                for (int i = split2end; i < ec; i++) edges1.Add(edges[i]);

                edges2.Add(edge1b);
                for (int i = split1end; i < split2beg; i++) edges2.Add(edges[i]);
                edges2.Add(edge2b);
                edges2.Add(splitb);
