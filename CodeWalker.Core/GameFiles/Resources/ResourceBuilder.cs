...
namespace CodeWalker.GameFiles
{
    public class ResourceBuilder
    {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    else if (blockset.Count > 0)
                    {
                        ...
                        block = blockset.FindBestBlock(long.MaxValue);
                ...
                if ((pageCount == pageFlags.Count) && (pageFlags.Size >= currentPosition) && (pageCount <= maxPageCount))
                {
                    break;
                }
        ...
        public static void AssignPositions2(IList<IResourceBlock> blocks, uint basePosition, out RpfResourcePageFlags pageFlags, uint maxPageCount, bool gen9)
        {
            ...
            var sys = (basePosition == 0x50000000);
            var maxPageSizeMult = 16L;
            var maxPageSize = (0x2000 << 0xF) * maxPageSizeMult;
            var maxBlockSize = 0L;
            ...
            var blockPages = new Dictionary<IResourceBlock, (int, int, long)>();
            while (true)
            {
                for (int i = 0; i < 5; i++)
                {
                ...
                for (int i = 0; i < sortedBlocks.Count; i++)
                {
                    ...
                    if (i == 0)
                    {
                        pageSizes[largestPageSizeI] = new List<long>() { size };
