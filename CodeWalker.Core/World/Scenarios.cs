...
namespace CodeWalker.World
{
    public class Scenarios
    {
        ...
        public void Init(GameFileCache gameFileCache, Action<string> updateStatus, Timecycle timecycle)
        {
            ...
            if ((manifestymt != null) && (manifestymt.CScenarioPointManifest != null))
            {
                ...
                {
                    string regionfilename = region.Name.ToString() + ".ymt";
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class ScenarioRegion : BasePathData
    {
        ...
        private void LoadTypes(MCScenarioPointRegion r, MCScenarioPoint scp)
        {
            ...
            var types = Scenarios.ScenarioTypes;
            if (types == null)
            { return; }
            ...
            if (tpind < typhashes.Length)
            {
                ...
                isveh = scp.Type?.IsVehicle ?? false;
            }
            else
            { }
                    ...
                    if ((r.Paths.Chains != null) && (r.Paths.Edges != null))
                    {
                        foreach (var chain in r.Paths.Chains)
                        {
                            ...
                            foreach (var edge in chain.Edges)
                            {
                                ...
                                pv1.Colour = (uint)new Color(cr1, cg, cb1, (byte)255).ToRgba();
                                pv2.Colour = (uint)new Color(cr2, cg, cb2, (byte)255).ToRgba();
                                pathverts.Add(pv1);
                                pathverts.Add(pv2);
                            }
                        }
        ...
        public ScenarioNode AddNode(ScenarioNode copy = null)
        {
            ...
            else
            {
                ...
                n.MyPoint.InteriorName = 493038497;
        ...
        public void RebuildLookUps()
        {
            ...
            Dictionary<MetaHash, int> typeNames = new Dictionary<MetaHash, int>();
            Dictionary<MetaHash, int> pedModelSetNames = new Dictionary<MetaHash, int>();
            Dictionary<MetaHash, int> vehicleModelSetNames = new Dictionary<MetaHash, int>();
            Dictionary<MetaHash, int> interiorNames = new Dictionary<MetaHash, int>();
            Dictionary<MetaHash, int> groupNames = new Dictionary<MetaHash, int>();
            Dictionary<MetaHash, int> imapNames = new Dictionary<MetaHash, int>();
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class ScenarioTypeRef
    {
        ...
        public bool IsVehicle => IsGroup ? false : Type.IsVehicle;
