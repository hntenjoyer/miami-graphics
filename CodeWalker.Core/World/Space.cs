...
namespace CodeWalker.World
{
    public class Space
    {
        ...
        public List<Entity> EnabledEntities = new List<Entity>();
        ...
        private void InitNodeGrid()
        {
            ...
            foreach (var rpffile in GameFileCache.BaseRpfs)
            {
        ...
        public void Update(float elapsed)
        {
            ...
            float gravamt = -9.8f;
            Vector3 dvgrav = new Vector3(0, 0, gravamt * elapsed);
            dvgrav += (0.5f * dvgrav * elapsed);
            float minvel = 0.5f;
        ...
        public SpaceEntityCollision FindFirstCollision(Entity e, float elapsed)
        {
            ...
            if (!r.SphereHit.Hit)
            {
                if (absdisp > e.Radius)
                {
                    ...
                    float rayl = absdisp + e.Radius * 4.0f;
        ...
        public SpaceRayIntersectResult RayIntersectEntity(ref Ray ray, YmapEntityDef ent, float maxdist = float.MaxValue)
        {
            ...
            if (drawable != null)
            {
                ...
                else if (drawable is FragDrawable fdrawable)
                {
                    ...
                    if (fbound != null)
                    {
                        var fhit = fbound.RayIntersect(ref eray, res.HitDist);
                        if (fhit.Hit)
                        {
                            fhit.Position = eori.Multiply(fhit.Position) + ent.Position;
                            fhit.Normal = eori.Multiply(fhit.Normal);
        ...
        public SpaceRayIntersectResult RayIntersectInterior(ref Ray ray, YmapEntityDef mlo, float maxdist = float.MaxValue)
        {
            ...
            {
                for (int j = 0; j < mlodat.Entities.Length; j++)
                {
                    var intent = mlodat.Entities[j];
                    if (intent.Archetype == null) continue;
        ...
        public SpaceSphereIntersectResult SphereIntersectEntity(ref BoundingSphere sph, YmapEntityDef ent)
        {
            ...
            if (drawable != null)
            {
                ...
                else if (drawable is FragDrawable fdrawable)
                {
                    ...
                    if (fbound != null)
                    {
                        var fhit = fbound.SphereIntersect(ref esph);
                        if (fhit.Hit)
                        {
                            fhit.Position = eori.Multiply(fhit.Position) + ent.Position;
                            fhit.Normal = eori.Multiply(fhit.Normal);
    ...
    public class SpaceMapDataStore
    {
        ...
        public List<MapDataStoreNode> GetItems(ref Vector3 min, ref Vector3 max)
        {
            VisibleItems.Clear();
        ...
        public List<MapDataStoreNode> GetItems(ref Ray ray)
        {
            VisibleItems.Clear();
    ...
    public class SpaceMapDataStoreNode
    {
        ...
        public void GetItems(ref Vector3 min, ref Vector3 max, List<MapDataStoreNode> items)
        {
        ...
        public void GetItems(ref Ray ray, List<MapDataStoreNode> items)
        {
    ...
    public class SpaceNavGrid
    {
        ...
        public Vector3 GetCellRel(Vector3 p)
        {
    ...
    public struct SpaceEntityCollision
    {
        ...
        public SpaceSphereIntersectResult SphereHit;
        public Vector3 PrePos;
        public float PreT;
        public float HitT;
        public Vector3 HitPos;
        public Quaternion HitRot;
        public Vector3 HitVel;
        public Vector3 HitAngVel;
